...
SH2Interface_struct SH2Interpreter = {SH2CORE_INTERPRETER,
                                      "SH2 Interpreter",
...
SH2Interface_struct SH2DebugInterpreter = {SH2CORE_DEBUGINTERPRETER,
...
static INLINE void SH2HandleInterrupts(SH2_struct *context)
{
    ...
    if (context->NumberOfInterrupts != 0) {
        if (context->interrupts[context->NumberOfInterrupts - 1].level > context->regs.SR.part.I) {
            ...
            // context->isslave?"SH2-S":"SH2-M", 9, context->interrupts[context->NumberOfInterrupts - 1].vector, oldpc,
            // context->regs.PC);
...
{
    if ((u32)sh->regs.R[INSTRUCTION_B(sh->instruction)] > (u32)sh->regs.R[INSTRUCTION_C(sh->instruction)])
        sh->regs.SR.part.T = 1;
    else
        sh->regs.SR.part.T = 0;
    sh->regs.PC += 2;
...
static void FASTCALL SH2movll(SH2_struct *sh)
{
    sh->regs.R[INSTRUCTION_B(sh->instruction)] =
        SH2MappedMemoryReadLong(sh, sh->regs.R[INSTRUCTION_C(sh->instruction)]);
    sh->regs.PC += 2;
    sh->cycles++;
}
...
{
    sh->regs.R[INSTRUCTION_B(sh->instruction)] =
        SH2MappedMemoryReadLong(sh, sh->regs.R[INSTRUCTION_C(sh->instruction)] + sh->regs.R[0]);
    sh->regs.PC += 2;
    sh->cycles++;
}
...
#ifdef SH2_UBC
        if (context->onchip.BBRA &
            (BBR_CPA_CPU | BBR_IDA_INST | BBR_RWA_READ))    // Break on cpu, instruction, read cycles
        {
...
int SH2InterpreterGetInterrupts(SH2_struct *context, interrupt_struct interrupts[MAX_INTERRUPTS])
{
