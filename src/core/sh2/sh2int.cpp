...
void SH2HandleInterrupts(SH2_struct *context)
{
    ...
    if (context->NumberOfInterrupts != 0) {
        if (context->interrupts[context->NumberOfInterrupts - 1].level > context->regs.SR.part.I) {
            ...
            context->regs.PC = SH2MappedMemoryReadLong(
                context, context->regs.VBR + (context->interrupts[context->NumberOfInterrupts - 1].vector << 2));
...
FASTCALL void SH2KronosInterpreterExecLoop(SH2_struct *context, u32 cycles)
{
    ...
    while (context->cycles < target_cycle) {
        cacheCode[context->isslave][cacheId[(context->regs.PC >> 20) & 0xFFF]][(context->regs.PC >> 1) & 0x7FFFF](
            context);
    }
}
...
void SH2KronosInterpreterSendInterrupt(SH2_struct *context, u8 vector, u8 level)
{
    ...
    if ((context == SSH2) && (yabsys.IsSSH2Running == 0))
        return;
...
int SH2KronosInterpreterGetInterrupts(SH2_struct *context, interrupt_struct interrupts[MAX_INTERRUPTS])
{
...
SH2Interface_struct SH2KronosInterpreter = {
    SH2CORE_KRONOS_INTERPRETER, "SH2 Kronos Interpreter",

    SH2KronosInterpreterInit, SH2KronosInterpreterDeInit, SH2KronosInterpreterReset, SH2KronosInterpreterExec,
    // SH2KronosDebugInterpreterExec,
    SH2KronosInterpreterTestExec,

    SH2KronosInterpreterGetRegisters, SH2KronosInterpreterGetGPR, SH2KronosInterpreterGetSR, SH2KronosInterpreterGetGBR,
    SH2KronosInterpreterGetVBR, SH2KronosInterpreterGetMACH, SH2KronosInterpreterGetMACL, SH2KronosInterpreterGetPR,
    SH2KronosInterpreterGetPC,

    SH2KronosInterpreterSetRegisters, SH2KronosInterpreterSetGPR, SH2KronosInterpreterSetSR, SH2KronosInterpreterSetGBR,
    SH2KronosInterpreterSetVBR, SH2KronosInterpreterSetMACH, SH2KronosInterpreterSetMACL, SH2KronosInterpreterSetPR,
    SH2KronosInterpreterSetPC, SH2KronosIOnFrame,

    SH2KronosInterpreterSendInterrupt, SH2KronosInterpreterRemoveInterrupt, SH2KronosInterpreterGetInterrupts,
    SH2KronosInterpreterSetInterrupts,

    SH2KronosWriteNotify, SH2KronosInterpreterAddCycles};
