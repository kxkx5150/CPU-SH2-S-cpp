...
void YglTmPull(YglTextureManager *tm, u32 flg)
{
    ...
    if (tm->texture == NULL) {
        ...
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, tm->pixelBufferID);
        if (flg) {
            tm->texture = (unsigned int *)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, tm->width * tm->height * 4,
                                                           GL_MAP_WRITE_BIT);
        } else {
            tm->texture = (unsigned int *)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, tm->width * tm->height * 4,
...
static void YglTMRealloc(YglTextureManager *tm, unsigned int width, unsigned int height)
{
    ...
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, new_pixelBufferID);
    new_texture =
        (unsigned int *)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, width * height * 4,
                                         GL_MAP_INVALIDATE_BUFFER_BIT | GL_MAP_WRITE_BIT | GL_MAP_UNSYNCHRONIZED_BIT);
...
static void YglTMAllocate_in(YglTextureManager *tm, YglTexture *output, unsigned int w, unsigned int h, unsigned int *x,
                             unsigned int *y)
{
    if (tm->width < w) {
...
void YglTMAllocate(YglTextureManager *tm, YglTexture *output, unsigned int w, unsigned int h, unsigned int *x,
                   unsigned int *y)
{
    YabThreadLock(tm->mtx);
    YglTMAllocate_in(tm, output, w, h, x, y);
...
{
    if ((temp & 0x8000) != 0)
        return ((u32)temp & 0x1F) << 3 | ((u32)temp & 0x3E0) << 6 | ((u32)temp & 0x7C00) << 9 | 0x10000;
    else
...
void VIDOGLVdp1WriteFrameBuffer(u32 type, u32 addr, u32 val)
{
    ...
    switch (type) {
        case 0:
            ...
            if (addr & 0x1)
                full = (full & 0xFF00) | (val & 0xFF);
            else
                full = (full & 0xFF) | ((val & 0xFF) << 8);
    ...
    switch (type) {
        ...
        case 1:
            *(u16 *)out = T1ReadLong((u8 *)_Ygl->vdp1fb_buf_read, addr * 2) & 0xFFFF;
            break;
        case 2:
            *(u32 *)out = ((T1ReadLong((u8 *)_Ygl->vdp1fb_buf_read, addr * 2) & 0xFFFF) << 16) |
                          ((T1ReadLong((u8 *)_Ygl->vdp1fb_buf_read, addr * 2 + 4) & 0xFFFF));
            break;
        default:
            break;
    }
...
void YglGenReset()
{
    ...
    if (YglTM_vdp1[0] != NULL)
        YglTMDeInit(&YglTM_vdp1[0]);
    if (YglTM_vdp1[1] != NULL)
...
void initLevels(YglLevel **levels, int size)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        for (j = 0; j < level[i].prgcount; j++) {
            ...
            if ((level[i].prg[j].quads = (float *)malloc(level[i].prg[j].maxQuad * sizeof(float))) == NULL) {
                return;
            }
            if ((level[i].prg[j].textcoords = (float *)malloc(level[i].prg[j].maxQuad * sizeof(float) * 2)) == NULL) {
                return;
            }
            if ((level[i].prg[j].vertexAttribute = (float *)malloc(level[i].prg[j].maxQuad * sizeof(float) * 2)) ==
                NULL) {
                return;
            }
        }
...
#ifdef _OGL3_
    if (maj * 10 + min < 43) {
        YuiMsg("Your graphic card is supporting only OpenGL %d.%d, you might suffer some graphical glitches depending "
               "your options\n",
               maj, min);
    }
    if (maj * 10 + min < 33) {
...
YglProgram *YglGetProgram(YglSprite *input, int prg, YglTextureManager *tm, int prio)
{
    ...
    if (input->uclipmode != level->uclipcurrent ||
        (input->uclipmode != 0 && (level->ux1 != Vdp1Regs->userclipX1 || level->uy1 != Vdp1Regs->userclipY1 ||
    ...
    if (level->prg[level->prgcurrent].prgid != prg) {
        ...
        level->prg[level->prgcurrent].id        = input->idScreen;
        level->prg[level->prgcurrent].blendmode = input->blendmode;
    } else if ((level->prg[level->prgcurrent].systemClipX2 != Vdp1Regs->systemclipX2) ||
...
static int YglQuadGrowShading_in(YglSprite *input, YglTexture *output, float *colors, YglCache *c, int cash_flg,
                                 YglTextureManager *tm);
static int YglQuadGrowShading_tesselation_in(YglSprite *input, YglTexture *output, float *colors, YglCache *c,
                                             int cash_flg, YglTextureManager *tm);
                              ...
                              YglTextureManager *tm)
{
    ...
    if (program == NULL || program->quads == NULL)
        return -1;
    ...
    texv[0].r = texv[1].r = texv[2].r = texv[3].r = texv[4].r = texv[5].r = 0;
    texv[0].q = texv[1].q = texv[2].q = texv[3].q = texv[4].q = texv[5].q = 1.0f;
...

    {
        ...
        for (v = 0; v < tess_count; v++) {
            ...
            for (u = 0; u < tess_count; u++) {
                ...
                ctpos[0].r = ctpos[1].r = ctpos[2].r = ctpos[3].r = ctpos[4].r = ctpos[5].r = 0;
                ctpos[0].q = ctpos[1].q = ctpos[2].q = ctpos[3].q = ctpos[4].q = ctpos[5].q = 1.0f;
                ...
                if (colors == NULL) {
                    ...
                    int uindex = u;
                    int vindex = v;
                    vtxa[0]    = (colors[0] * (tess_count - uindex) + colors[4] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[12] * (tess_count - u) + colors[8] * uindex) / (float)tess_count * vindex;
                    vtxa[1] = (colors[1] * (tess_count - uindex) + colors[5] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[13] * (tess_count - u) + colors[9] * uindex) / (float)tess_count * vindex;
                    vtxa[2] = (colors[2] * (tess_count - uindex) + colors[6] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[14] * (tess_count - u) + colors[10] * uindex) / (float)tess_count * vindex;
                    vtxa[3] = (colors[3] * (tess_count - uindex) + colors[7] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[15] * (tess_count - u) + colors[11] * uindex) / (float)tess_count * vindex;
                    ...
                    uindex  = u + 1;
                    vindex  = v;
                    vtxa[4] = (colors[0] * (tess_count - uindex) + colors[4] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[12] * (tess_count - u) + colors[8] * uindex) / (float)tess_count * vindex;
                    vtxa[5] = (colors[1] * (tess_count - uindex) + colors[5] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[13] * (tess_count - u) + colors[9] * uindex) / (float)tess_count * vindex;
                    vtxa[6] = (colors[2] * (tess_count - uindex) + colors[6] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[14] * (tess_count - u) + colors[10] * uindex) / (float)tess_count * vindex;
                    vtxa[7] = (colors[3] * (tess_count - uindex) + colors[7] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[15] * (tess_count - u) + colors[11] * uindex) / (float)tess_count * vindex;
                    ...
                    uindex  = u + 1;
                    vindex  = v + 1;
                    vtxa[8] = (colors[0] * (tess_count - uindex) + colors[4] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[12] * (tess_count - u) + colors[8] * uindex) / (float)tess_count * vindex;
                    vtxa[9] = (colors[1] * (tess_count - uindex) + colors[5] * uindex) / (float)tess_count *
                                  (tess_count - vindex) +
                              (colors[13] * (tess_count - u) + colors[9] * uindex) / (float)tess_count * vindex;
                    vtxa[10] = (colors[2] * (tess_count - uindex) + colors[6] * uindex) / (float)tess_count *
                                   (tess_count - vindex) +
                               (colors[14] * (tess_count - u) + colors[10] * uindex) / (float)tess_count * vindex;
                    vtxa[11] = (colors[3] * (tess_count - uindex) + colors[7] * uindex) / (float)tess_count *
                                   (tess_count - vindex) +
                               (colors[15] * (tess_count - u) + colors[11] * uindex) / (float)tess_count * vindex;
                    ...
                    uindex   = u;
                    vindex   = v + 1;
                    vtxa[20] = (colors[0] * (tess_count - uindex) + colors[4] * uindex) / (float)tess_count *
                                   (tess_count - vindex) +
                               (colors[12] * (tess_count - u) + colors[8] * uindex) / (float)tess_count * vindex;
                    vtxa[21] = (colors[1] * (tess_count - uindex) + colors[5] * uindex) / (float)tess_count *
                                   (tess_count - vindex) +
                               (colors[13] * (tess_count - u) + colors[9] * uindex) / (float)tess_count * vindex;
                    vtxa[22] = (colors[2] * (tess_count - uindex) + colors[6] * uindex) / (float)tess_count *
                                   (tess_count - vindex) +
                               (colors[14] * (tess_count - u) + colors[10] * uindex) / (float)tess_count * vindex;
                    vtxa[23] = (colors[3] * (tess_count - uindex) + colors[7] * uindex) / (float)tess_count *
                                   (tess_count - vindex) +
                               (colors[15] * (tess_count - u) + colors[11] * uindex) / (float)tess_count * vindex;
                          ...
                          YglTextureManager *tm)
{
    ...
    if (c != NULL && cash_flg == 1) {
        switch (input->flip) {
            case 0:
                c->x = *(program->textcoords + ((program->currentQuad - 12) * 2));
                c->y = *(program->textcoords + ((program->currentQuad - 12) * 2) + 1);
                break;
            case 1:
                c->x = *(program->textcoords + ((program->currentQuad - 10) * 2));
                c->y = *(program->textcoords + ((program->currentQuad - 10) * 2) + 1);
                break;
            case 2:
                c->x = *(program->textcoords + ((program->currentQuad - 2) * 2));
                c->y = *(program->textcoords + ((program->currentQuad - 2) * 2) + 1);
                break;
            case 3:
                c->x = *(program->textcoords + ((program->currentQuad - 4) * 2));
                c->y = *(program->textcoords + ((program->currentQuad - 4) * 2) + 1);
                break;
        }
    }
                                      ...
                                      YglTextureManager *tm)
{
    ...
    tmp[0].r = tmp[1].r = tmp[2].r = tmp[3].r = 0.0f;
    tmp[0].q = tmp[1].q = tmp[2].q = tmp[3].q = 1.0f;
...
static void YglQuadOffset_in(vdp2draw_struct *input, YglTexture *output, YglCache *c, int cx, int cy, float sx,
                             float sy, int cash_flg, YglTextureManager *tm);
...
void YglQuadOffset_in(vdp2draw_struct *input, YglTexture *output, YglCache *c, int cx, int cy, float sx, float sy,
                      int cash_flg, YglTextureManager *tm)
{
    unsigned int              x, y;
    YglProgram               *program;
...
int YglQuadRbg0(RBGDrawInfo *rbg, YglTexture *output, YglCache *c, int rbg_type, YglTextureManager *tm,
                Vdp2 *varVdp2Regs)
{
    unsigned int              x, y;
    YglProgram               *program;
...
void YglEraseWriteVDP1(int id)
{
    ...
    if (color != 0x0) {
        if (((Vdp1Regs->TVMR & 0x1) == 1) && (col[0] != col[1])) {
            YuiMsg("Unsupported clear process\n\tin 8 bits upper part of EWDR is for even coordinates and lower part "
                   "for odd coordinates\n");
        }
    }
...
static int renderVDP1Level(YglLevel *level, int j, int *cprg, YglMatrix *mat, Vdp2 *varVdp2Regs)
{
    ...
    if (level->prg[j].currentQuad != 0) {
        ...
        glBufferData(GL_ARRAY_BUFFER, level->prg[j].currentQuad * sizeof(float) * 2, level->prg[j].textcoords,
                     GL_STREAM_DRAW);
        ...
        if (level->prg[j].vaid > 0) {
            ...
            glBufferData(GL_ARRAY_BUFFER, level->prg[j].currentQuad * sizeof(float) * 2, level->prg[j].vertexAttribute,
                         GL_STREAM_DRAW);
...
u8 *YglGetVDP2RegPointer()
{
    ...
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, _Ygl->vdp2reg_pbo);
    _Ygl->vdp2reg_buf =
        (u8 *)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, 512 * NB_VDP2_REG,
...
int DrawVDP2Screen(Vdp2 *varVdp2Regs, int id)
{
    ...
    for (int j = 0; j < (level->prgcurrent + 1); j++) {
        if (level->prg[j].currentQuad != 0) {
            ...
            glBindBuffer(GL_ARRAY_BUFFER, _Ygl->quads_buf);
            glBufferData(GL_ARRAY_BUFFER, level->prg[j].currentQuad * sizeof(float), level->prg[j].quads,
                         GL_STREAM_DRAW);
            glVertexAttribPointer(level->prg[j].vertexp, 2, GL_FLOAT, GL_FALSE, 0, 0);
            glEnableVertexAttribArray(level->prg[j].vertexp);
            ...
            if (level->prg[j].vaid > 0) {
                glBindBuffer(GL_ARRAY_BUFFER, _Ygl->vertexAttribute_buf);
                glBufferData(GL_ARRAY_BUFFER, level->prg[j].currentQuad * sizeof(float) * 2,
                             level->prg[j].vertexAttribute, GL_STREAM_DRAW);
                glVertexAttribPointer(level->prg[j].vaid, 4, GL_FLOAT, GL_FALSE, 0, 0);
                glEnableVertexAttribArray(level->prg[j].vaid);
            }
...
SpriteMode setupBlend(Vdp2 *varVdp2Regs, int layer)
{
    ...
    if ((layer == 6) && ((Vdp1Regs->TVMR & 0x1) == 1))
        return NONE;
...
void YglRender(Vdp2 *varVdp2Regs)
{
    ...
    for (int j = 0; j < 6; j++) {
        if (drawScreen[vdp2screens[j]] != 0) {
            ...
            if (vdp2screens[j] == RBG0)
                useLineColorOffset[id] = _Ygl->useLineColorOffset[0];
            if (vdp2screens[j] == RBG1)
                useLineColorOffset[id] = _Ygl->useLineColorOffset[1];
  ...
  for (int i = 0; i < 8; i++) {
    ...
    glDrawBuffers(NB_RENDER_LAYER, (const GLenum *)&DrawBuffers[0]);
    glClearBufferfi(GL_DEPTH_STENCIL, 0, 0, 0);
    YglBlitTexture(prioscreens, modescreens, isRGB, isBlur, isPerline, isShadow, lncl_draw, getVDP1fb, winS_draw,
                   winS_mode_draw, win0_draw, win0_mode_draw, win1_draw, win1_mode_draw, win_op_draw,
                   useLineColorOffset, varVdp2Regs);
...
static void updateColorRamLine(int lineStart, int start, int end)
{
    ...
    start_addr                     = (start >> index_shft);
    size                           = ((end - start) >> index_shft) + 1;
    glTexSubImage2D(GL_TEXTURE_2D, 0, start_addr, lineStart, size, 1, GL_RGBA, GL_UNSIGNED_BYTE, &buf[start_addr]);
}
...
void YglUpdateColorRam()
{
    ...
    for (int l = 0; l < yabsys.MaxLineCount - 1; l++) {
        if ((Vdp2ColorRamUpdated[l] != 0) && (l != 0))
            needToSync = 1;
...
u32 *YglGetLineColorScreenPointer()
{
    ...
    glBindTexture(GL_TEXTURE_2D, _Ygl->linecolorscreen_tex);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, _Ygl->linecolorscreen_pbo);
    _Ygl->linecolorscreen_buf =
        (u32 *)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, 512 * 4,
                                GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT | GL_MAP_UNSYNCHRONIZED_BIT);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
...
void setupMaxSize()
{
    ...
    if ((_Ygl->rwidth != 0) && (_Ygl->width > GlWidth))
        _Ygl->width = _Ygl->rwidth * (GlWidth / _Ygl->rwidth + 1);
    if ((_Ygl->rheight != 0) && (_Ygl->height > GlHeight))
        _Ygl->height = _Ygl->rheight * (GlHeight / _Ygl->rheight + 1);
    if (oldWidth != _Ygl->width)
        rebuild_frame_buffer = 1;
    if (oldHeight != _Ygl->height)
        rebuild_frame_buffer = 1;
}
...
void YglChangeResolution(int w, int h)
{
    ...
    if (_Ygl->vdp2wdensity / _Ygl->vdp2hdensity != 1.0) {
        uh = h * _Ygl->vdp2wdensity;
        uw = w * _Ygl->vdp2hdensity;
    }
    int maxRes = GlHeight;
    ...
    YGLDEBUG("YglChangeResolution %dx%d => %d => %dx%d (%.1f,%.1f) (%d %d)\n", w, h, scale, _Ygl->width, _Ygl->height,
             _Ygl->vdp2wdensity, _Ygl->vdp2hdensity, uw, uh);
    ...
    if (_Ygl->rheight >= 448)
        _Ygl->heightRatio *= 2.0f;
    if (_Ygl->rwidth >= 640)
        _Ygl->widthRatio *= 2.0f;
...
u32 *YglGetPerlineBuf(void)
{
    ...
    glBindTexture(GL_TEXTURE_2D, _Ygl->coloroffset_tex);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, _Ygl->coloroffset_pbo);
    _Ygl->coloroffset_buf =
        (u32 *)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, 512 * 4 * 8,
                                GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT | GL_MAP_UNSYNCHRONIZED_BIT);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
