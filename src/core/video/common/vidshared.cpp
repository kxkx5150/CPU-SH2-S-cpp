...
void Vdp2ReadRotationTable(int which, vdp2rotationparameter_struct *parameter, Vdp2 *regs, u8 *ram)
{
    ...
    if (parameter->coefenab) {
        ...
        if (regs->RAMCTL & 0x8000) {
            parameter->k_mem_type = 1;
            i                     = Vdp2RamReadLong(NULL, ram, addr);
            ftmp = (float)(signed)((i & 0x03FFFFC0) | (i & 0x02000000 ? 0xFE000000 : 0x00000000)) / 65536;
            parameter->deltaKAx = ftmp;
        } else {
            parameter->k_mem_type = 0;
        }
    ...
    VDP2LOG("Xst: %f, Yst: %f, Zst: %f, deltaXst: %f deltaYst: %f deltaX: %f\n", parameter->Xst, parameter->Yst,
            parameter->Zst, parameter->deltaXst, parameter->deltaYst, parameter->deltaX);
    VDP2LOG("deltaY: %f A: %f B: %f C: %f D: %f E: %f F: %f Px: %f Py: %f Pz: %f\n", parameter->deltaY, parameter->A,
            parameter->B, parameter->C, parameter->D, parameter->E, parameter->F, parameter->Px, parameter->Py,
            parameter->Pz);
    VDP2LOG("Cx: %f Cy: %f Cz: %f Mx: %f My: %f kx: %f ky: %f KAst: %f\n", parameter->Cx, parameter->Cy, parameter->Cz,
            parameter->Mx, parameter->My, parameter->kx, parameter->ky, parameter->KAst);
...
void Vdp2ReadRotationTableFP(int which, vdp2rotationparameterfp_struct *parameter, Vdp2 *regs, u8 *ram)
{
    ...
    if (parameter->coefenab) {
        ...
        if (which == 0) {
            ...
            parameter->coeftbladdr =
                ((regs->KTAOF & 0x7) * 0x10000 + touint(parameter->KAst)) * parameter->coefdatasize;
            parameter->coefmode = (regs->KTCTL >> 2) & 0x3;
        } else {
            parameter->coefdatasize = (regs->KTCTL & 0x200 ? 2 : 4);
            parameter->coeftbladdr =
                (((regs->KTAOF >> 8) & 0x7) * 0x10000 + touint(parameter->KAst)) * parameter->coefdatasize;
    ...
    VDP2LOG("Xst: %f, Yst: %f, Zst: %f, deltaXst: %f deltaYst: %f deltaX: %f\n"
            ...
            tofloat(parameter->Xst), tofloat(parameter->Yst), tofloat(parameter->Zst), tofloat(parameter->deltaXst),
            tofloat(parameter->deltaYst), tofloat(parameter->deltaX), tofloat(parameter->deltaY), tofloat(parameter->A),
            tofloat(parameter->B), tofloat(parameter->C), tofloat(parameter->D), tofloat(parameter->E),
            tofloat(parameter->F), tofloat(parameter->Px), tofloat(parameter->Py), tofloat(parameter->Pz),
            tofloat(parameter->Cx), tofloat(parameter->Cy), tofloat(parameter->Cz), tofloat(parameter->Mx),
            tofloat(parameter->My), tofloat(parameter->kx), tofloat(parameter->ky), tofloat(parameter->KAst),
