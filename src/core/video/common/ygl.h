...
int  YglInitShader(int id, const GLchar *vertex[], int vcount, const GLchar *frag[], int fcount, const GLchar *tc[],
                   const GLchar *te[], const GLchar *g[]);
...
extern int YglBlitTexture(int *prioscreens, int *modescreens, int *isRGB, int *isBlur, int *isPerline, int *isShadow,
...
#define SHADER_VERSION_COMPUTE "#version 430 core \n"
    ...
    !defined(_OGLES31_) && !defined(__LIBRETRO__)
...
extern void(STDCALL *glVertexAttribPointer)(GLuint index, GLint size, GLenum type, GLboolean normalized, GLsizei stride,
                                            const void *pointer);
