...
{
    if (id == 0)
        return get_vdp1_tex(_Ygl->readframe);
    else
        return get_vdp1_mesh(_Ygl->readframe);
}
...
void YglCSRender(Vdp2 *varVdp2Regs)
{
    ...
    YglBlitTexture(prioscreens, modescreens, isRGB, isBlur, isPerline, isShadow, lncl_draw, GetCSVDP1fb, winS_draw,
