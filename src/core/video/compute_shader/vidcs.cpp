...
VideoInterface_struct VIDCS = {VIDCORE_CS,
...
void addCSCommands(vdp1cmd_struct *cmd, int type)
{
    ...
    YuiMsg("\t[%d,%d]\n", cmd->CMDXD, cmd->CMDYD);
    YuiMsg("\n\n");
    YuiMsg("=> %d (%d %d %d %d => %d %d) %f %f %f %f\n", cmd->nbStep, ADx, ADy, BCx, BCy, nbStepAD, nbStepBC,
           cmd->uAstepx, cmd->uAstepy, cmd->uBstepx, cmd->uBstepy);
    YuiMsg("==============\n");
#endif
    vdp1_add(cmd, 0);
}
...
void VIDCSVdp1DistortedSpriteDraw(vdp1cmd_struct *cmd, u8 *ram, Vdp1 *regs, u8 *back_framebuffer)
{
    ...
    if (getBestMode(cmd) == DISTORTED) {
        ...
        cmd->type = QUAD;
        if (cmd->CMDXA <= cmd->CMDXB)
            cmd->CMDXB += 1;
        else
            cmd->CMDXB -= 1;
        if (cmd->CMDXD <= cmd->CMDXC)
            cmd->CMDXC += 1;
        else
            cmd->CMDXC -= 1;
        if (cmd->CMDYB <= cmd->CMDYC)
            cmd->CMDYC += 1;
        else
            cmd->CMDYC -= 1;
        if (cmd->CMDYA <= cmd->CMDYD)
            cmd->CMDYD += 1;
        else
            cmd->CMDYD -= 1;
        vdp1_add(cmd, 0);
...
void VIDCSVdp1UserClipping(vdp1cmd_struct *cmd, u8 *ram, Vdp1 *regs)
{
    if ((cmd->CMDXC > regs->systemclipX2) && (cmd->CMDYC > regs->systemclipY2)) {
        regs->localX = 0;
        regs->localY = 0;
    }
...
void VIDCSVdp1SystemClipping(vdp1cmd_struct *cmd, u8 *ram, Vdp1 *regs)
{
    if (((cmd->CMDXC) == regs->systemclipX2) && (regs->systemclipY2 == (cmd->CMDYC)))
        return;
