...
#define COLOR_ADD(l, r, g, b)                                                                                          \
    (COLOR_ADDb((l >> 24) & 0xFF, r) << 24) | (COLOR_ADDb((l >> 16) & 0xFF, g) << 16) |                                \
        (COLOR_ADDb((l >> 8) & 0xFF, b) << 8) | (l & 0xFF)
#else
#define COLOR_ADD(l, r, g, b)                                                                                          \
    COLOR_ADDb((l & 0xFF), r) | (COLOR_ADDb((l >> 8) & 0xFF, g) << 8) | (COLOR_ADDb((l >> 16) & 0xFF, b) << 16) |      \
        (l & 0xFF000000)
#endif
...
VideoInterface_struct VIDOGL = {VIDCORE_OGL,
...
vdp2rotationparameter_struct *FASTCALL vdp2RGetParamMode02WithKAWithKB(RBGDrawInfo *rgb, int h, int v,
                                                                       Vdp2 *varVdp2Regs);
...
static void FASTCALL Vdp1ReadTexture_in_sync(vdp1cmd_struct *cmd, int spritew, int spriteh, YglTexture *texture,
                                             Vdp2 *varVdp2Regs)
{
    int shadow        = 0;
    int normalshadow  = 0;
        ...
        case 0: {
            ...
            for (i = 0; i < spriteh; i++) {
                ...
                while (j < spritew) {
                    ...
                    if (((dot >> 4) == 0) && (!SPD))
                        *texture->textdata++ = VDP1COLOR(endTag, 0x00);
                    else
                        *texture->textdata++ = VDP1COLOR(endTag, ((dot >> 4) | colorBank));
                    j += 1;
                    if ((!END) && (endcnt >= 2)) {
                        endTag = 0xFF;
                    ...
                    if (((dot & 0xF) == 0) && (!SPD))
                        ...
                        *texture->textdata++ = VDP1COLOR(endTag, ((dot & 0xF) | colorBank));
                    j += 1;
                    charAddr += 1;
                }
            ...
            for (i = 0; i < spriteh; i++) {
                ...
                for (j = 0; j < spritew; j++) {
                    ...
                    if ((dot == 0) && (!SPD))
                        ...
                        *texture->textdata++ = VDP1COLOR(endTag, ((dot & 0x3F) | colorBank));
                }
                texture->textdata += texture->w;
            }
            break;
            ...
            for (i = 0; i < spriteh; i++) {
                ...
                for (j = 0; j < spritew; j++) {
                    ...
                    if ((dot == 0) && (!SPD))
                        ...
                        *texture->textdata++ = VDP1COLOR(endTag, ((dot & 0x7F) | colorBank));
                }
                texture->textdata += texture->w;
            }
            break;
            ...
            for (i = 0; i < spriteh; i++) {
                ...
                for (j = 0; j < spritew; j++) {
                    ...
                    if ((dot == 0) && (!SPD))
                        ...
                        *texture->textdata++ = VDP1COLOR(endTag, (dot | colorBank));
                }
                texture->textdata += texture->w;
            }
            break;
...
void Vdp2GenerateWindowInfo(Vdp2 *varVdp2Regs)
{
    ...
    for (int i = 0; i < enBGMAX + 1; i++) {
        if (Win0[i] != _Ygl->Win0[i])
            _Ygl->needWinUpdate |= 1;
        if (Win1[i] != _Ygl->Win1[i])
            _Ygl->needWinUpdate |= 1;
        if (WinS[i] != _Ygl->WinS[i])
            _Ygl->needWinUpdate |= 1;
        if (Win0_mode[i] != _Ygl->Win0_mode[i])
            _Ygl->needWinUpdate |= 1;
        if (Win1_mode[i] != _Ygl->Win1_mode[i])
            _Ygl->needWinUpdate |= 1;
        if (WinS_mode[i] != _Ygl->WinS_mode[i])
            _Ygl->needWinUpdate |= 1;
...
void Vdp2GenLineinfo(vdp2draw_struct *info)
{
    ...
    for (i = 0; i < _Ygl->rheight; i += info->lineinc) {
        index = 0;
        if (VDPLINE_SX(info->islinescroll)) {
            info->lineinfo[lineindex].LineScrollValH =
                Vdp2RamReadWord(NULL, Vdp2Ram, info->linescrolltbl + (i / info->lineinc) * bound);
        ...
        if (VDPLINE_SY(info->islinescroll)) {
            info->lineinfo[lineindex].LineScrollValV =
                Vdp2RamReadWord(NULL, Vdp2Ram, info->linescrolltbl + (i / info->lineinc) * bound + index);
...
static INLINE u32 Vdp2GetPixel16bppbmp(vdp2draw_struct *info, u32 addr, Vdp2 *varVdp2Regs)
{
    ...
    if (!(dot & 0x8000) && info->transparencyenable)
        ...
        color = VDP2COLOR(info->idScreen, info->alpha, info->priority, cc, RGB555_TO_RGB24(dot));
    return color;
}
...
static INLINE u32 Vdp2GetPixel32bppbmp(vdp2draw_struct *info, u32 addr, Vdp2 *varVdp2Regs)
{
    ...
    if (!(dot1 & 0x8000) && info->transparencyenable)
        ...
        color = VDP2COLOR(info->idScreen, info->alpha, info->priority, cc,
                          (((dot1 & 0xFF) << 16) | (dot2 & 0xFF00) | (dot2 & 0xFF)));
    return color;
}
...
static u32 getAlpha(vdp2draw_struct *info, int id)
{
    ...
    if (idx < 0)
        idx = 0;
    if ((idx >> vdp2_interlace) > yabsys.VBlankLineCount)
        idx = yabsys.VBlankLineCount << vdp2_interlace;
                            ...
                            *texture->textdata++ = 0x00000000;
                            ...
                            switch (info->specialcolormode) {
                                ...
                                case 2:
                                    ...
                                    break;
                                case 3:
                                    if (((Vdp2ColorRamGetColorRaw(color) & 0x8000) == 0)) {
                                        cc = 0;
                                    }
                                    break;
                            }
...
static void Vdp2DrawPatternPos(vdp2draw_struct *info, YglTexture *texture, int x, int y, int cx, int cy, int lines,
                               Vdp2 *varVdp2Regs)
{
    u64 cacheaddr = ((u32)(info->alpha_per_line[y >> vdp2_interlace] >> 3) << 27) | (info->paladdr << 20) |
                    info->charaddr | info->transparencyenable | ((info->patternpixelwh >> 4) << 1) |
                    (((u64)(info->coloroffset >> 8) & 0x07) << 32) | (((u64)(info->idScreen) & 0x07) << 39);
    ...
    switch (info->auxmode) {
        case 0:
            ...
            switch (info->patternwh) {
                ...
                case 2:
                    info->charaddr =
                        ((tmp & 0x3FF) << 2) | (info->supplementdata & 0x3) | ((info->supplementdata & 0x1C) << 10);
                    break;
            }
            break;
        case 1:
            ...
            switch (info->patternwh) {
                ...
                case 2:
                    info->charaddr =
                        ((tmp & 0xFFF) << 2) | (info->supplementdata & 0x3) | ((info->supplementdata & 0x10) << 10);
                    break;
            }
            break;
    }
...
static int Vdp2PatternAddrPos(vdp2draw_struct *info, int planex, int x, int planey, int y, Vdp2 *varVdp2Regs)
{
    u32 addr = info->addr + (info->pagewh * info->pagewh * info->planew * planey +
    ...
    if (info->pname_bank[ptnAddrBk] == 0)
        return 0;
    ...
    switch (info->colornumber) {
        ...
        case 4:
            ...
            if (!(dot & 0x80000000) && info->transparencyenable)
                ...
                return VDP2COLOR(info->idScreen, alpha, info->priority, 1, dot & 0xFFFFFF);
        default:
            return 0;
    }
}
        ...
        for (h = -info->patternpixelwh; h < info->draww + info->patternpixelwh; h += info->patternpixelwh) {
            ...
            if (Vdp2PatternAddrPos(info, planex, pagex, planey, pagey, varVdp2Regs) != 0) {
                ...
                if (info->char_bank[charAddrBk] == 1) {
                    ...
                    int y           = v - chary;
                    info->draw_line = y;
                    if (delayed && (h == -info->patternpixelwh))
                        continue;
...
static void FASTCALL Vdp2DrawRotation(RBGDrawInfo *rbg, Vdp2 *varVdp2Regs)
{
    ...
    if (_Ygl->rheight >= 448)
        lineInc <<= 1;
    if (_Ygl->rheight >= 448)
        rbg->vres = (_Ygl->rheight >> 1);
    else
        rbg->vres = _Ygl->rheight;
    if (_Ygl->rwidth >= 640)
        rbg->hres = (_Ygl->rwidth >> 1);
    else
        rbg->hres = _Ygl->rwidth;
...
static void Vdp2DrawRotation_in_sync(RBGDrawInfo *rbg, Vdp2 *varVdp2Regs)
{
    ...
    if (rbg->useb) {
        ...
        rbg->paraB.Xp = rbg->paraB.A * (rbg->paraB.Px - rbg->paraB.Cx) +
                        rbg->paraB.B * (rbg->paraB.Py - rbg->paraB.Cy) +
                        rbg->paraB.C * (rbg->paraB.Pz - rbg->paraB.Cz) + rbg->paraB.Cx + rbg->paraB.Mx;
        rbg->paraB.Yp = rbg->paraB.D * (rbg->paraB.Px - rbg->paraB.Cx) +
                        rbg->paraB.E * (rbg->paraB.Py - rbg->paraB.Cy) +
...
void VIDOGLReadColorOffset(void)
{
    ...
    for (int line = 0; line < _Ygl->rheight; line++) {
        ...
        if (lVdp2Regs->COAB & 0x100)
            a_cob |= 0xFFFFFF00;
        int colOffB = (((int)(128.0f + (b_cob / 2.0)) & 0xFF) << 16) | (((int)(128.0f + (b_cog / 2.0)) & 0xFF) << 8) |
                      (((int)(128.0f + (b_cor / 2.0)) & 0xFF) << 0);
        int colOffA = (((int)(128.0f + (a_cob / 2.0)) & 0xFF) << 16) | (((int)(128.0f + (a_cog / 2.0)) & 0xFF) << 8) |
...
void VIDOGLVdp1UserClipping(vdp1cmd_struct *cmd, u8 *ram, Vdp1 *regs)
{
    if ((cmd->CMDXC + 1 > regs->systemclipX2) && (cmd->CMDYC + 1 > regs->systemclipY2)) {
        regs->localX = 0;
        regs->localY = 0;
    }
...
static void Vdp2DrawBackScreen(Vdp2 *varVdp2Regs)
{
    ...
    if ((varVdp2Regs->BKTAU & 0x8000) != 0) {
        ...
        YglSetClearColor((float)(((dot & 0x1F) << 3) + info.cor) / (float)(0xFF),
...
{
    if ((a->BGON & 0x1010) != (b->BGON & 0x1010))
        return 0;
...
int sameVDP2Reg(int id, Vdp2 *a, Vdp2 *b)
{
    switch (id) {
        case RBG0:
            return sameVDP2RegRBG0(a, b);
        case RBG1:
            return sameVDP2RegRBG1(a, b);
        default:
            break;
    }
    return 1;
