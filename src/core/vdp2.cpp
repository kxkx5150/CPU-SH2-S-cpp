...
void FASTCALL Vdp2RamWriteByte(SH2_struct *context, u8 *mem, u32 addr, u8 val)
{
    ...
    if (A0_Updated == 0 && addr >= 0 && addr < (0x20000 << (Vdp2Regs->VRSIZE >> 15))) {
        A0_Updated = 1;
    } else if (A1_Updated == 0 && addr >= (0x20000 << (Vdp2Regs->VRSIZE >> 15)) &&
               addr < (0x40000 << (Vdp2Regs->VRSIZE >> 15))) {
        A1_Updated = 1;
    } else if (B0_Updated == 0 && addr >= (0x40000 << (Vdp2Regs->VRSIZE >> 15)) &&
               addr < (0x60000 << (Vdp2Regs->VRSIZE >> 15))) {
        B0_Updated = 1;
    } else if (B1_Updated == 0 && addr >= (0x60000 << (Vdp2Regs->VRSIZE >> 15)) &&
               addr < (0x80000 << (Vdp2Regs->VRSIZE >> 15))) {
        B1_Updated = 1;
    }
...
void       Vdp2HBlankOUT(void)
{
    ...
    if (yabsys.LineCount < yabsys.VBlankLineCount) {
        ...
        for (i = 0; i < 88; i++) {
            cell_scroll_data[yabsys.LineCount].data[i] =
                Vdp2RamReadLong(NULL, Vdp2Ram, cell_scroll_table_start_addr + i * 4);
        }
