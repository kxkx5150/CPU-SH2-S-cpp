...
void  *YabMemMap(char *filename, u32 size)
{
    ...
    hFile = CreateFileA(filename, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    ...
    hFMWrite = CreateFileMapping(hFile, NULL, PAGE_READWRITE, 0, size, "BACKUP");
...
static void FASTCALL UnhandledMemoryWriteByte(SH2_struct *context, UNUSED u8 *memory, USED_IF_DEBUG u32 addr,
                                              UNUSED u8 val)
{
...
static void FASTCALL UnhandledMemoryWriteWord(SH2_struct *context, UNUSED u8 *memory, USED_IF_DEBUG u32 addr,
                                              UNUSED u16 val)
{
...
static void FASTCALL UnhandledMemoryWriteLong(SH2_struct *context, UNUSED u8 *memory, USED_IF_DEBUG u32 addr,
                                              UNUSED u32 val)
{
...
static u32 FASTCALL BupRamMemoryReadLong(SH2_struct *context, UNUSED u8 *memory, USED_IF_DEBUG u32 addr)
{
    ...
    return ((BupRamMemoryReadByte(context, memory, addr | 0x1) << 8) ||
            (BupRamMemoryReadByte(context, memory, addr | 0x3) << 16));
}
...
static void FASTCALL BupRamMemoryWriteWord(SH2_struct *context, UNUSED u8 *memory, USED_IF_DEBUG u32 addr,
                                           UNUSED u16 val)
{
...
static void FASTCALL BupRamMemoryWriteLong(SH2_struct *context, UNUSED u8 *memory, USED_IF_DEBUG u32 addr,
                                           UNUSED u32 val)
{
...
static void FillMemoryArea(unsigned short start, unsigned short end, readbytefunc r8func, readwordfunc r16func,
                           readlongfunc r32func, writebytefunc w8func, writewordfunc w16func, writelongfunc w32func,
                           u8 **memory)
{
    int i;
...
void MappedMemoryInit()
{
    ...
    FillMemoryArea(0x000, 0xFFF, &UnhandledMemoryReadByte, &UnhandledMemoryReadWord, &UnhandledMemoryReadLong,
                   &UnhandledMemoryWriteByte, &UnhandledMemoryWriteWord, &UnhandledMemoryWriteLong, &VoidMem);

    // Fill the rest
    FillMemoryArea(0x000, 0x00F, &BiosRomMemoryReadByte, &BiosRomMemoryReadWord, &BiosRomMemoryReadLong,
                   &BiosRomMemoryWriteByte, &BiosRomMemoryWriteWord, &BiosRomMemoryWriteLong, &BiosRom);
    FillMemoryArea(0x010, 0x017, &SmpcReadByte, &SmpcReadWord, &SmpcReadLong, &SmpcWriteByte, &SmpcWriteWord,
                   &SmpcWriteLong, &VoidMem);
    FillMemoryArea(0x020, 0x02F, &LowWramMemoryReadByte, &LowWramMemoryReadWord, &LowWramMemoryReadLong,
                   &LowWramMemoryWriteByte, &LowWramMemoryWriteWord, &LowWramMemoryWriteLong, &LowWram);
    FillMemoryArea(0x040, 0x041, &IOPortReadByte, &IOPortReadWord, &UnhandledMemoryReadLong, &IOPortWriteByte,
                   &UnhandledMemoryWriteWord, &UnhandledMemoryWriteLong, &VoidMem);
    FillMemoryArea(0x100, 0x17F, &UnhandledMemoryReadByte, &UnhandledMemoryReadWord, &UnhandledMemoryReadLong,
                   &UnhandledMemoryWriteByte, &SSH2InputCaptureWriteWord, &UnhandledMemoryWriteLong, &VoidMem);
    FillMemoryArea(0x180, 0x1FF, &UnhandledMemoryReadByte, &UnhandledMemoryReadWord, &UnhandledMemoryReadLong,
                   &UnhandledMemoryWriteByte, &MSH2InputCaptureWriteWord, &UnhandledMemoryWriteLong, &VoidMem);
    FillMemoryArea(0x200, 0x3FF, CartridgeArea->Cs0ReadByte, CartridgeArea->Cs0ReadWord, CartridgeArea->Cs0ReadLong,
                   CartridgeArea->Cs0WriteByte, CartridgeArea->Cs0WriteWord, CartridgeArea->Cs0WriteLong,
                   &CartridgeArea->rom);    // A voir ici
    FillMemoryArea(0x400, 0x4FF, &Cs1ReadByte, &Cs1ReadWord, &Cs1ReadLong, &Cs1WriteByte, &Cs1WriteWord, &Cs1WriteLong,
                   &VoidMem);    // A voir ici
    FillMemoryArea(0x580, 0x58F, &Cs2ReadByte, &Cs2ReadWord, &Cs2ReadLong, &Cs2WriteByte, &Cs2WriteWord, &Cs2WriteLong,
                   &VoidMem);    // A voir ici
    FillMemoryArea(0x5A0, 0x5AF, &SoundRamReadByte, &SoundRamReadWord, &SoundRamReadLong, &SoundRamWriteByte,
                   &SoundRamWriteWord, &SoundRamWriteLong, &SoundRam);
    FillMemoryArea(0x5B0, 0x5BF, &scsp_r_b, &scsp_r_w, &scsp_r_d, &scsp_w_b, &scsp_w_w, &scsp_w_d, &VoidMem);
    FillMemoryArea(0x5C0, 0x5C7, &Vdp1RamReadByte, &Vdp1RamReadWord, &Vdp1RamReadLong, &Vdp1RamWriteByte,
                   &Vdp1RamWriteWord, &Vdp1RamWriteLong, &Vdp1Ram);
    FillMemoryArea(0x5C8, 0x5CF, &Vdp1FrameBufferReadByte, &Vdp1FrameBufferReadWord, &Vdp1FrameBufferReadLong,
                   &Vdp1FrameBufferWriteByte, &Vdp1FrameBufferWriteWord, &Vdp1FrameBufferWriteLong, &VoidMem);
    FillMemoryArea(0x5D0, 0x5D7, &Vdp1ReadByte, &Vdp1ReadWord, &Vdp1ReadLong, &Vdp1WriteByte, &Vdp1WriteWord,
                   &Vdp1WriteLong, &VoidMem);
    FillMemoryArea(0x5E0, 0x5EF, &Vdp2RamReadByte, &Vdp2RamReadWord, &Vdp2RamReadLong, &Vdp2RamWriteByte,
                   &Vdp2RamWriteWord, &Vdp2RamWriteLong, &Vdp2Ram);
    FillMemoryArea(0x5F0, 0x5F7, &Vdp2ColorRamReadByte, &Vdp2ColorRamReadWord, &Vdp2ColorRamReadLong,
                   &Vdp2ColorRamWriteByte, &Vdp2ColorRamWriteWord, &Vdp2ColorRamWriteLong, &Vdp2ColorRam);
    FillMemoryArea(0x5F8, 0x5FB, &Vdp2ReadByte, &Vdp2ReadWord, &Vdp2ReadLong, &Vdp2WriteByte, &Vdp2WriteWord,
                   &Vdp2WriteLong, &VoidMem);
    FillMemoryArea(0x5FE, 0x5FE, &ScuReadByte, &ScuReadWord, &ScuReadLong, &ScuWriteByte, &ScuWriteWord, &ScuWriteLong,
                   &VoidMem);
    FillMemoryArea(0x600, 0x7FF, &HighWramMemoryReadByte, &HighWramMemoryReadWord, &HighWramMemoryReadLong,
                   &HighWramMemoryWriteByte, &HighWramMemoryWriteWord, &HighWramMemoryWriteLong, &HighWram);

    FillMemoryArea(((backup_file_addr >> 16) & 0xFFF),
                   (((backup_file_addr + (backup_file_size << 1)) >> 16) & 0xFFF) + 1, &BupRamMemoryReadByte,
                   &BupRamMemoryReadWord, &BupRamMemoryReadLong, &BupRamMemoryWriteByte, &BupRamMemoryWriteWord,
                   &BupRamMemoryWriteLong, &BupRam);
}
...
void FASTCALL MappedMemoryWriteByte(SH2_struct *context, u32 addr, u8 val)
{
    ...
    if (context == NULL)
        id = 1;
    if ((context != NULL) && (context->cacheOn == 0))
...
{
    return (((addr & 0xFFFFF) == 0x7d600) ||
            (((addr & 0xFFFFF) >= 0x0384 && (addr & 0xFFFFF) <= 0x03A8) || ((addr & 0xFFFFF) == 0x358)));
}
...
int BackupInit(const char *path, int extended)
{
    ...
    if (currentSaveSize != -1) {
        ...
        if (extended && isNormalSize) {
            ...
            YuiMsg("Internal backup file format is detected as standard save format - Force to use standard save "
                   "format\n");
        }
        if (!extended && isExtendedSize) {
            extended = 1;    // Force to use large save format
            YuiMsg("Internal backup file format is detected as extended save format - Force to use extended save "
                   "format\n");
        }
...
int LoadBios(const char *filename)
{
    ...
    if (yabsys.isSTV)
        YuiMsg("ST-V Emulation mode\n");
    else
        YuiMsg("Saturn Emulation mode\n");
    return ret;
}
...
static u8 header[16] = {'B', 'a', 'c', 'k', 'U', 'p', 'R', 'a', 'm', ' ', 'F', 'o', 'r', 'm', 'a', 't'};
...
static int MappedMemoryAddMatch(SH2_struct *sh, u32 addr, u32 val, int searchtype, result_struct *result,
                                u32 *numresults)
{
...
static INLINE int SearchIncrementAndCheckBounds(result_struct *prevresults, u32 *maxresults, u32 numresults, u32 *i,
                                                u32 inc, u32 *newaddr, u32 endaddr)
{
    if (prevresults) {
        if (i[0] >= maxresults[0]) {
...
static int SearchString(SH2_struct *sh, u32 startaddr, u32 endaddr, int searchtype, const char *searchstr,
                        result_struct *results, u32 *maxresults)
{
    u8  *buf    = NULL;
    u32 *buf32  = NULL;
    u32  buflen = 0;
...
result_struct *MappedMemorySearch(SH2_struct *sh, u32 startaddr, u32 endaddr, int searchtype, const char *searchstr,
                                  result_struct *prevresults, u32 *maxresults)
{
    ...
    switch (searchtype & 0x70) {
        ...
        case SEARCHREL16BIT: {
            ...
            if (SearchString(sh, startaddr, endaddr, searchtype, searchstr, results, maxresults) == 0) {
                maxresults[0] = 0;
                free(results);
                return NULL;
            }
