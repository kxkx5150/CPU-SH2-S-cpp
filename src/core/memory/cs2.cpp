...
u16 FASTCALL Cs2ReadWord(SH2_struct *context, UNUSED u8 *memory, u32 addr)
{
    ...
    switch (addr) {
        ...
        case 0x0C:
        case 0x0E:
            return Cs2Area->reg.HIRQMASK;
        case 0x18:
        case 0x1A:
            return Cs2Area->reg.CR1;
        case 0x1C:
        case 0x1E:
            return Cs2Area->reg.CR2;
        case 0x20:
        case 0x22:
            return Cs2Area->reg.CR3;
        case 0x24:
        case 0x26:
            Cs2Area->_command = 0;
                ...
                case 5:
                    ...
                    if (Cs2Area->datatranstype != CDB_DATATRANSTYPE_INVALID) {
                        ...
                        if (Cs2Area->datanumsecttrans < Cs2Area->datasectstotrans) {
                            ...
                            if (Cs2Area->datatranspartition
                                    ->block[Cs2Area->datatranssectpos + Cs2Area->datanumsecttrans] == NULL) {
...
u32 FASTCALL Cs2ReadLong(SH2_struct *context, UNUSED u8 *memory, u32 addr)
{
    ...
    switch (addr) {
        case 0x08:
            ...
            return val;
        case 0x0C:
            return ((Cs2Area->reg.HIRQMASK << 16) | Cs2Area->reg.HIRQMASK);
        case 0x18:
            return ((Cs2Area->reg.CR1 << 16) | Cs2Area->reg.CR1);
        case 0x1C:
            return ((Cs2Area->reg.CR2 << 16) | Cs2Area->reg.CR2);
        case 0x20:
            return ((Cs2Area->reg.CR3 << 16) | Cs2Area->reg.CR3);
        case 0x24:
            Cs2Area->_command = 0;
            ...
            if (Cs2Area->datatranstype != CDB_DATATRANSTYPE_INVALID) {
                ...
                if (Cs2Area->datanumsecttrans < Cs2Area->datasectstotrans) {
                    u8 *ptr = &Cs2Area->datatranspartition->block[Cs2Area->datatranssectpos + Cs2Area->datanumsecttrans]
                                   ->data[Cs2Area->datatransoffset];
                    if (Cs2Area->datatranspartition->block[Cs2Area->datatranssectpos + Cs2Area->datanumsecttrans] ==
                        NULL) {
                        CDLOG("cs2\t: datatranspartition->block[Cs2Area->datanumsecttrans] was NULL");
                        return 0;
                    }
...
void Cs2Execute(void)
{
    ...
    switch (instruction) {
        ...
        case 0x03:
            ...
            break;
        case 0x04:
            CDLOG("cs2\t: Command: initializeCDSystem %04x %04x %04x %04x %04x\n", Cs2Area->reg.HIRQ, Cs2Area->reg.CR1,
                  Cs2Area->reg.CR2, Cs2Area->reg.CR3, Cs2Area->reg.CR4);
        ...
        case 0x94:
            CDLOG("cs2\t: Command: mpegSetMode %04x %04x %04x %04x\n", Cs2Area->reg.HIRQ, Cs2Area->reg.CR1,
                  Cs2Area->reg.CR2, Cs2Area->reg.CR3);
            Cs2MpegSetMode();
            break;
        case 0x95:
            CDLOG("cs2\t: Command: mpegPlay %04x %04x %04x %04x\n", Cs2Area->reg.HIRQ, Cs2Area->reg.CR1,
                  Cs2Area->reg.CR2, Cs2Area->reg.CR4);
            Cs2MpegPlay();
            break;
        case 0x96:
...
void Cs2GetSectorInfo(void)
{
    ...
    if (gsibufno < MAX_SELECTORS) {
        if (gsisctnum < Cs2Area->partition[gsibufno].numblocks) {
            Cs2Area->reg.CR1 =
                (u16)((Cs2Area->status << 8) | ((Cs2Area->partition[gsibufno].block[gsisctnum]->FAD >> 16) & 0xFF));
            Cs2Area->reg.CR2 = (u16)Cs2Area->partition[gsibufno].block[gsisctnum]->FAD;
            Cs2Area->reg.CR3 = (Cs2Area->partition[gsibufno].block[gsisctnum]->fn << 8) |
                               Cs2Area->partition[gsibufno].block[gsisctnum]->cn;
            Cs2Area->reg.CR4 = (Cs2Area->partition[gsibufno].block[gsisctnum]->sm << 8) |
                               Cs2Area->partition[gsibufno].block[gsisctnum]->ci;
            Cs2SetIRQ(CDB_HIRQ_CMOK | CDB_HIRQ_ESEL);
            return;
        } else {
    ...
    if (psdbufno < MAX_SELECTORS) {
        ...
        if (psdsectnum > Cs2Area->blockfreespace)
            ...
            for (i = 0; i < psdsectnum; i++) {
                putpartition->block[putpartition->numblocks] =
                    Cs2AllocateBlock(&putpartition->blocknum[putpartition->numblocks], Cs2Area->putsectsize);
...
void Cs2CopySectorData(void)
{
    ...
    for (int i = 0; i < count; i++) {
        putpartition->block[putpartition->numblocks] =
            Cs2AllocateBlock(&putpartition->blocknum[putpartition->numblocks], 2352);
...
void Cs2ChangeDirectory(void)
{
    ...
    if (cdfilternum == 0xFF) {
        ...
        if (Cs2ReadFileSystem(Cs2Area->filter + cdfilternum, ((Cs2Area->reg.CR3 & 0xFF) << 16) | Cs2Area->reg.CR4, 0) !=
            0) {
...
void Cs2ReadDirectory(void)
{
    ...
    if (rdfilternum == 0xFF) {
        ...
        if (Cs2ReadFileSystem(Cs2Area->filter + rdfilternum, ((Cs2Area->reg.CR3 & 0xFF) << 8) | Cs2Area->reg.CR4, 1) !=
            0) {
            CDLOG("cs2\t: ReadFileSystem failed\n");
            doCDReport(CDB_STAT_REJECT);
            Cs2SetIRQ(CDB_HIRQ_CMOK | CDB_HIRQ_EFLS);
...
void Cs2ReadFile(void)
{
    ...
    rfsize      = ((Cs2Area->fileinfo[rffid].size + Cs2Area->getsectsize - 1) / Cs2Area->getsectsize) - rfoffset;
    ...
    if (Cs2Area->mpegpath && (mpgfp = fopen(Cs2Area->mpegpath, "rb")) != NULL) {
        ...
        if ((mpgpartition = Cs2GetPartition(Cs2Area->outconmpegrom)) != NULL && !Cs2Area->isbufferfull) {
            ...
            for (i = 0; i < readsize; i++) {
                mpgpartition->block[mpgpartition->numblocks] =
        ...
        switch (Cs2Area->getsectsize) {
            case 2048:    // user data only
                if (Cs2Area->workblock.data[0xF] == 0x02) {
                    ...
                    memcpy(rufspartition->block[rufspartition->numblocks]->data, Cs2Area->workblock.data + 16, 2048);
                    Cs2Area->workblock.size = Cs2Area->getsectsize;
                }
                break;
...
u8 Cs2GetIP(int autoregion)
{
    ...
    if ((gripartition = Cs2ReadUnFilteredSector(150)) != NULL) {
        ...
        if (memcmp(buf, "SEGA SEGASATURN", 15) == 0) {
            ...
            cdip->version[6] = '\0';
            sprintf(cdip->date, "%c%c/%c%c/%c%c%c%c", buf[0x34], buf[0x35], buf[0x36], buf[0x37], buf[0x30], buf[0x31],
                    buf[0x32], buf[0x33]);
...
#else
            cdip->ipsize        = (buf[0xE0] << 24) | (buf[0xE1] << 16) | (buf[0xE2] << 8) | buf[0xE3];
            cdip->msh2stack     = (buf[0xE8] << 24) | (buf[0xE9] << 16) | (buf[0xEA] << 8) | buf[0xEB];
