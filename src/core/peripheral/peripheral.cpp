...
const char *PerPadNames[] = {"Up", "Right", "Down", "Left", "R", "L", "Start", "A", "B", "C", "X", "Y", "Z", NULL};

const char *PerMouseNames[] = {"A", "B", "C", "Start", NULL};
    ...
    switch (addr) {
        ...
        case 0x0d:
            if (m_ioga_mode & 0x80) {
                ...
                m_ioga_portg = (m_ioga_portg & 0xf8) | ((m_ioga_portg + 1) & 7);
            } else
                val = IOPORT[PORT_G];
            break;
        case 0x1b:
...
void PerMouseMove(PerMouse_struct *mouse, s32 dispx, s32 dispy)
{
    ...
    if (negx)
        diffx = ~(mouse->mousebits[1]) & 0xFF;
    else
        diffx = mouse->mousebits[1];
    if (negy)
        diffy = ~(mouse->mousebits[2]) & 0xFF;
    else
        diffy = mouse->mousebits[2];
    ...
    mouse->mousebits[0] =
        (overflowy << 7) | (overflowx << 6) | (negy << 5) | (negx << 4) | (mouse->mousebits[0] & 0x0F);
    if (negx)
        mouse->mousebits[1] = ~(diffx);
    else
        mouse->mousebits[1] = diffx;
    if (negy)
        mouse->mousebits[2] = ~(diffy);
    else
        mouse->mousebits[2] = diffy;
}
...
void PerAxis1Value(PerAnalog_struct *analog, u32 val)
{
    ...
    if (analog->perid == PERWHEEL) {
        ...
        if (val <= 0x67)
            analog->analogbits[0] &= 0xBF;
        else if (left_is_pressed && val >= 0x6f)
            analog->analogbits[0] |= ~0xBF;

        if (val >= 0x97)
            analog->analogbits[0] &= 0x7F;
        else if (right_is_pressed && val <= 0x8f)
            analog->analogbits[0] |= ~0x7F;
    } else if (analog->perid == PERMISSIONSTICK || analog->perid == PERTWINSTICKS) {
        int left_is_pressed  = (analog->analogbits[0] & (1 << 6)) == 0;
        int right_is_pressed = (analog->analogbits[0] & (1 << 7)) == 0;
...
void *PerAddPeripheral(PortData_struct *port, int perid)
{
    ...
    switch (perid) {
        ...
        case PERTWINSTICKS:
            PerUpdateConfig(perpadbaseconfig, sizeof(perpadbaseconfig) / sizeof(PerBaseConfig_struct), controller);
            PerUpdateConfig(peranalogbaseconfig, sizeof(peranalogbaseconfig) / sizeof(PerBaseConfig_struct),
                            controller);
            break;
        case PERGUN:
...
PerInterface_struct PERDummy = {
    PERCORE_DUMMY, "Dummy Input Interface", PERDummyInit, PERDummyDeInit, PERDummyHandleEvents, PERDummyScan, 0,
    PERDummyFlush, PERDummyKeyName};
