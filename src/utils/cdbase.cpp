...
CDInterface DummyCD = {
    CDCORE_DUMMY,   "Dummy CD Drive",     DummyCDInit,         DummyCDDeInit,    DummyCDGetStatus,
    DummyCDReadTOC, DummyCDReadSectorFAD, DummyCDReadAheadFAD, DummyCDSetStatus,
};
...
CDInterface ISOCD = {
    CDCORE_ISO,   "ISO-File Virtual Drive", ISOCDInit,         ISOCDDeInit,    ISOCDGetStatus,
    ISOCDReadTOC, ISOCDReadSectorFAD,       ISOCDReadAheadFAD, ISOCDSetStatus,
};
    ...
    for (i = 0; i < nbFiles; i++) {
        if (strncasecmp(filename, fileListTemp[i]->d_name, l) == 0) {
            ...
            for (k = 0; k < strlen(path); k++) {
                if (shallBeEscaped(path[k]))
                    *tmp++ = '\\';
                *tmp++ = path[k];
            }
            *tmp++ = '/';
            for (k = 0; k < strlen(fileListTemp[i]->d_name); k++) {
                if (shallBeEscaped(fileListTemp[i]->d_name[k]))
                    *tmp++ = '\\';
...
static int LoadBinCue(const char *cuefilename, FILE *iso_file)
{
    ...
    for (;;) {
        ...
        if (strncmp(temp_buffer, "FILE", 4) == 0) {
            ...
            current_file_id++;
            if (track_num > 0) {
                trk[track_num - 1].fad_end =
                    trk[track_num - 1].fad_start +
                    (trk[track_num - 1].file_size - trk[track_num - 1].file_offset) / trk[track_num - 1].sector_size;
        ...
        if (strncmp(temp_buffer, "TRACK", 5) == 0) {
            ...
            if (strncmp(temp_buffer, "MODE1", 5) == 0 || strncmp(temp_buffer, "MODE2", 5) == 0) {
...
int deflateFile(JZFile *zip, int idx, JZFileHeader *header, char *filename, void *user_data)
{
    ...
    if (entry->filename == NULL) {
        ...
        if (last != NULL) {
            ...
            CDLOG("uncompressed %d %f %f\n", header->uncompressedSize, header->uncompressedSize / 2448.0,
                  header->uncompressedSize / 2352.0);
...
static int GetIntCCD(ccd_struct *ccd, char *section, char *name)
{
    ...
    for (i = 0; i < ccd->num_dict; i++) {
        ...
        if (strcasecmp(ccd->dict[i].section, section) == 0 && strcasecmp(ccd->dict[i].name, name) == 0)
#else
        if (stricmp(ccd->dict[i].section, section) == 0 && stricmp(ccd->dict[i].name, name) == 0)
#endif
...
static int LoadCCD(const char *ccd_filename, FILE *iso_file)
{
    ...
    for (i = 0; i < num_toc; i++) {
        ...
        if (point == 0xA1) {
            ...
            ;
            disc.session[ses - 1].track =
                (track_info_struct *)malloc(disc.session[ses - 1].track_num * sizeof(track_info_struct));
    ...
    for (i = 0; i < num_toc; i++) {
        ...
        if (point >= 1 && point <= 99) {
            ...
            if (point >= 2) {
                ...
                disc.session[ses - 1].track[point - 2].file_size = (disc.session[ses - 1].track[point - 2].fad_end + 1 -
                                                                    disc.session[ses - 1].track[point - 2].fad_start) *
                                                                   2352;
            }
...
static int         ISOCDReadSectorFAD(u32 FAD, void *buffer)
{
    ...
    for (i = 0; i < disc.session_num; i++) {
        for (j = 0; j < disc.session[i].track_num; j++) {
            if (FAD >= disc.session[i].track[j].fad_start && FAD <= disc.session[i].track[j].fad_end) {
    ...
    if (currentTrack->isZip != 1) {
        if (offset > currentTrack->file_size)
            offset = currentTrack->file_size;
    ...
    if (currentTrack->sector_size == 2448) {
        if (!currentTrack->interleaved_sub) {
            ...
            const u16 deint_offsets[] = {0,  66,  125, 191, 100, 50,  150, 175, 8,  33,  58,  83,  108, 133, 158, 183,
                                         16, 41,  25,  91,  116, 141, 166, 75,  24, 90,  149, 215, 124, 74,  174, 199,
                                         32, 57,  82,  107, 132, 157, 182, 207, 40, 65,  49,  115, 140, 165, 190, 99,
                                         48, 114, 173, 239, 148, 98,  198, 223, 56, 81,  106, 131, 156, 181, 206, 231,
                                         64, 89,  73,  139, 164, 189, 214, 123, 72, 138, 197, 263, 172, 122, 222, 247,
                                         80, 105, 130, 155, 180, 205, 230, 255, 88, 113, 97,  163, 188, 213, 238, 147};
...
static int LoadCHD(const char *chd_filename, FILE *iso_file)
{
    ...
    while (chd_get_metadata(pChdInfo->chd, 0, num_tracks, buf, meta_outlen, &resultlen, &resulttag, &resultflags) ==
           CHDERR_NONE) {
        ...
        switch (resulttag) {
            ...
            case CDROM_TRACK_METADATA2_TAG:
                sscanf((const char *)buf, CDROM_TRACK_METADATA2_FORMAT, &trak_number, track_type, track_subtype, &frame,
                       &pregap, pg_type, pg_sub_type, &postgap);
                break;
            default:
                return -1;
        }
